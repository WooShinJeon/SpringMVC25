...
@Controller
@RequestMapping("/sample/*")  //  http://192.168.111.104:80/controller/sample/모든것에 반응한다.
...
public class SampleController {
	...
	//  http://192.168.111.104:80/controller/sample/basic -> get 메서드와 post 메서드에 동작
	public void basicGet() {
		log.info("SampleController.basicGet() 메서드 실행.............");
		log.info("get 방식과 post 방식 둘다 반응한다.");
	} // basicGet() 메서드 종료
	...
	public String ex01(SampleDTO dto) { // SampleDTO dto = new SampleDTO();
		...
		return "mbcex01"; //  WEB-INF/views/ex01.jsp -> servlet-context.xml 이 하는 일
	} // ex01() 메서드 종료
	...
	public String ex02(@RequestParam("owner") String name, @RequestParam("age") int age) {
		// 프론트 변수와 백앤드의 변수가 다를 때!!! (초보자는 일치시킬것)
		log.info("name : " + name);
		log.info("age : " + age);
	...
	public String ex02Bean(SampleDTOList list) {
		...
		//   http://192.168.111.104:80/controller/sample/ex02Bean?list%5B0%5D.name=kkw&list%5B2%5D.age=33
	...
	public String ex04(SampleDTO dto, @ModelAttribute("page") int page) { // int page는 게시판에 페이징 기법
		// @ModelAttribute("page") 안쓰면 백앤드에는 전달이 되지만 프론트에는 전달이 안됨
