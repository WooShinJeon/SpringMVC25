...
public class BoardMapperTests {
	...
	public void testInsertSelectKey() {
		...
		BoardVO board = new BoardVO();
		board.setTitle("번호나와라 번호나와라 번호나와라 번호나와라");
		board.setContent("번호는? 번호는? 번호는? 번호는? 번호는? ");
		board.setWriter("김기원 김기원 김기원 김기원 ");
		
		mapper.insertSelectKey(board);
		
		log.info(board);
		log.info("내가만든게시물의번호는???????????? : " + board.getBno());
	}
